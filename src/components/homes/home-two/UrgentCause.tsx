...
const UrgentCause = async () => {
   try {
      const homepage = await client.fetch(`*[_type == "homepage"][0]`, {}, {
        next: { tags: ['homepage'], revalidate: 60 }
      });

      const urgentServiceImage = homepage?.urgentServiceImage;
      const urgentServiceImageUrl = urgentServiceImage ? urlFor(urgentServiceImage).url() : "/assets/img/clean/1.jpg";
      const UrgentServiceTitle = homepage?.UrgentServiceTitle || "Emergency Deep Cleaning Available Now";
      const UrgentServiceText = homepage?.UrgentServiceText || "When spills, stains, or unexpected messes happen, our team is ready to respond quickly. We offer fast and professional deep cleaning services for homes, offices, and commercial spaces. Your environment will look spotless, fresh, and healthy again — within hours.";
                  ...
                  <div className="row gap-40 align-items-center">
                     <div className="col-lg-6">
                        <div className="urgent-cause-left-image rmb-65">
                           <Image src={urgentServiceImageUrl} alt="Emergency Cleaning Service" width={400} height={500} />
                        </div>
                     </div>
